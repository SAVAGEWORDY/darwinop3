...
#include "op3_football_walking/op3_walking_module.h"
...
void WalkingModule::initialize(const int control_cycle_msec, robotis_framework::Robot *robot)
{
  ...
  std::string default_param_path = ament_index_cpp::get_package_share_directory("op3_football_walking") + "/config/param.yaml";
...
bool WalkingModule::computeLegAngle(double *leg_angle)
{
  ...
  // mm, rad
  double dynamic_x_offset = x_offset_;
  if (x_move_amplitude_ > 0)
  {
    dynamic_x_offset += (x_move_amplitude_ * 0.15);
  }
  else if (x_move_amplitude_ < 0)
  {
    dynamic_x_offset += (x_move_amplitude_ * 0.10);
  }

  ep[0] = swap.x + right_leg_move.x + dynamic_x_offset;
  ...
  ep[6] = swap.x + left_leg_move.x + dynamic_x_offset;
...
void WalkingModule::sensoryFeedback(const double &rlGyroErr, const double &fbGyroErr, double *balance_angle)
{
  ...
  double base_period = 0.6;
  if (walking_param_.period_time > 0.01)
  {
    internal_gain = 0.05 * (base_period/ walking_param_.period_time);
  }
  if (internal_gain > 0.12) internal_gain = 0.12;
